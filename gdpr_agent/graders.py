"""
LLM-based grading functions for GDPR Agent quality control.
Implements context relevance evaluation and answer groundedness verification.
"""

import json
import logging
import mlflow
from pydantic import BaseModel, Field
from typing import Optional
from . import config

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - [%(filename)s:%(lineno)d] - %(message)s'
)
logger = logging.getLogger(__name__)


class RelevanceGrade(BaseModel):
    """Schema for context relevance evaluation."""
    is_relevant: bool


class GroundednessGrade(BaseModel):
    """Schema for answer groundedness verification."""
    is_grounded: bool
    reason: str = Field(default='')


# ============================================================================
# UTILITY HELPER 1: Retrieval Grader
# ============================================================================
@mlflow.trace(name="grade_retrieval", span_type="LLM")
def grade_retrieved_context(user_question: str, retrieved_context: str) -> bool:
    """
    Evaluates whether the retrieved context is relevant and contains useful info
    to help answer the user's question.
    
    Args:
        user_question: The user's original question
        retrieved_context: The context retrieved from vector search
        
    Returns:
        True if context is relevant, False otherwise
        
    Raises:
        Exception: If OpenAI API call fails (re-raised after logging)
    """
    if not retrieved_context.strip():
        logger.debug("Empty retrieved context - returning False")
        return False
    
    logger.debug("Grading retrieved context relevance for question: %s", 
                user_question[:100] + "..." if len(user_question) > 100 else user_question)
    logger.debug("Context length: %d characters", len(retrieved_context))
    
    prompt = f"""You are a quality control auditor. Evaluate if the provided retrieved context contains background information, direct evidence, or context that is relevant to answering the user's question.

    Retrieved Context:
    {retrieved_context}

    User Question: {user_question}

    Provide your assessment in a strict JSON format with a single boolean key 'is_relevant' (true or false).
    JSON Output:"""

    try:
        logger.debug("Calling OpenAI API for context relevance grading")
        response = config.openai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": prompt}],
            response_format={"type": "json_object"},
            temperature=0.0,
            seed=42
        )
    except Exception as e:
        logger.exception("Failed to call OpenAI API for context grading: %s", e)
        raise
    
    try:
        result = RelevanceGrade(**json.loads(response.choices[0].message.content))
        logger.info("Context relevance grading completed: is_relevant=%s", result.is_relevant)
        return result.is_relevant
    except Exception as e:
        logger.error("Grader response validation error: %s", e)
        logger.debug("Response content: %s", response.choices[0].message.content)
        return False


# ============================================================================
# UTILITY HELPER 2: Hallucination & Groundedness Grader
# ============================================================================
@mlflow.trace(name="grade_answer_groundedness", span_type="LLM")
def grade_answer_groundedness(generated_answer: str, retrieved_context: str) -> bool:
    """
    Verifies that the generated answer is strictly supported by the context, 
    preventing hallucinations or outside assumptions.
    
    Args:
        generated_answer: The LLM-generated answer to verify
        retrieved_context: The source context used to generate the answer
        
    Returns:
        True if answer is grounded in context, False otherwise
        
    Raises:
        Exception: If OpenAI API call fails (re-raised after logging)
    """
    logger.debug("Verifying answer groundedness against source documents")
    logger.debug("Context length: %d characters", len(retrieved_context))
    logger.debug("Answer length: %d characters", len(generated_answer))
    
    prompt = f"""You are an expert compliance risk assessor. Compare the generated answer against the approved retrieved context. Determine if the generated answer contains ANY claims, assumptions, or facts that are NOT explicitly stated or directly backed by the retrieved context.

    Retrieved Context:
    {retrieved_context}

    Generated Answer:
    {generated_answer}

    Determine if the generated answer is substantially grounded in the context. Minor reasonable inferences for readability are acceptable.

    Provide your assessment in JSON format with:
    - "is_grounded": boolean (true if substantially supported by context, false if it contains major unsupported claims)
    - "reason": string (brief explanation of your decision)
    JSON Output:"""

    try:
        logger.debug("Calling OpenAI API for groundedness verification")
        response = config.openai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": prompt}],
            response_format={"type": "json_object"},
            temperature=0.0,
            seed=42
        )
    except Exception as e:
        logger.exception("Failed to call OpenAI API for groundedness grading: %s", e)
        raise

    try:
        result = GroundednessGrade(**json.loads(response.choices[0].message.content))
        logger.info("Groundedness grading completed: is_grounded=%s", result.is_grounded)
        logger.debug("Grader reasoning: %s", result.reason)
        return result.is_grounded
    except Exception as e:
        logger.error("Groundedness validation error: %s", e)
        logger.debug("Response content: %s", response.choices[0].message.content)
        return False
